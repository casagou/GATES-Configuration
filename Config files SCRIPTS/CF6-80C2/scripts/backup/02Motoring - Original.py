...
channel("FCS_AirRdy,FCS_FuelRdy,IGN1Pwr,IGN2Pwr,FuelOn,StartControl,StrtCtrlST,MFPump,MFPumpST,TLAFILTR,StartReset,StAirOn, StAirFB,AIRSTRT,StrtVlvST,N1,N2,POIL,ECUModeChaA,AECUModeST,ECUModeChaB,BECUModeST,ECUFltChaA, ECUFltChaB,WF")
...
set_channel("StartControl", 2)

wait("StrtCtrlST = 2", 3, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "Set StartMode switch in Manual mode. ")
...
if getCV("StAirFB") == 0:
	instruction("Set Start Air Supply ON.")

	set_channel("StAirOn", 0)

	delay(2)

	set_channel("StAirOn", 1)

	wait("StAirFB = 1", 10, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "Start Air not responding.  Use PLC")

	if SkipGV:
		result("Operator skipped Start Air ON instruction", REPORT + "Motoring", YELLOW)

	else:
		result("Start Air turned ON", REPORT + "Motoring")

		pass
else:
	result("Start Air was already ON", REPORT + "Motoring")

	pass

instruction("Energize Start valve.")
...
set_channel("AIRSTRT", 1)

wait("StrtVlvST=1", 5, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "Engine Start Air Valve not open within 5 sec")
...
note("Set both channel SELF TEST switches to GROUND TEST.")

set_channel("ECUModeChaA", 1)

wait("AECUModeST = 1", 5, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "ECU ChA Ground Test switch not responding.")

if SkipGV:
	result("Operator skipped ECU Cha B GROUND TEST switch", REPORT + "Motoring")

	pass

set_channel("ECUModeChaB", 1)

wait("BECUModeST = 1", 5, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "ECU ChA Ground Test switch not responding.")
...
lvtemp = prompt_boo("Do you want to do wet motoring?")
...
# ***** TESTING 002 PARA 2.B (4) *****
	...
	instruction("Turn Fuel ON")

	wait("FuelOn=1", 3, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "Fuel switch not on")
	...
	if getCV("WF") >200:
		result("Motoring Fuel Flow = {} lb/hr".format(str(round(getCV("WF"), 4)) ), REPORT +"Motoring")

	else:
		result("Motoring Fuel Flow = {} lb/hr - less than 200 pph".format(str(round(getCV("WF"), 4)) ), REPORT +"Motoring")
...
# ***** TESTING 002 PARA 2.B (5) *****
	...
	instruction("Turn Fuel Off, and continue to motor for 60 s")

	wait("FuelOn=0", 10, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "Fuel switch not off")
...
wait("StAirFB = 0", 10, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "Start Air not responding.  Use PLC")
...
set_channel("AIRSTRT", 0)

wait("StrtVlvST=0", 5, 0.1, WAIT_PARAM3_DFT, WAIT_PARAM4_DFT, WAIT_PARAM5_DFT, WAIT_PARAM6_DFT, WAIT_PARAM7_DFT, MSG, "Start Air Valve did not close in 5 sec")
