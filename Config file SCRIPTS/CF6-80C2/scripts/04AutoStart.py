...
#*  TESTING 003 TASK 72-00-00-760-003-C
#*
#*  MODIFICATIONS:
...
note("If engine was operated in the last 2 hours,do dry motoring for 90s")


# ***** TESTING 003 PARA 2.A (1) *****
...
# ***** TESTING 003 PARA 2.A (1) (a) ***** Test Cell Starter Inlet Air Pressure Available
...
# ***** TESTING 003 PARA 2.A (1) (b) (c)***** Test Cell Fuel Pressure Available
...
# ***** TESTING 003 PARA 2.A (1) (d) ***** Test Cell Engine Oil Quantity Available

result("Oil Quantity is {} L.".format(str(round(getCV("OILQTY"),4)) ),REPORT + "Start")


# ***** TESTING 003 PARA 2.A (1) (e) ***** Engine Minimum Idle selected
...
if getCV("IdleCtrlST") == 1:
	...
	if SkipGV:
		result("Operator skipped Min Idle mode.",REPORT + "Start")
...
# ***** TESTING 003 PARA 2.A (1) (f) ***** Ignition selection - Single
...
# ***** TESTING 003 PARA 2.A (1) (g) ***** Manual/Auto Start

instruction("Set AUTO/MANUAL SWITCH to AUTO")

set_channel("StartControl",1)

wait("AutoStrtST = 1",3,0.1,WAIT_PARAM3_DFT,WAIT_PARAM4_DFT,WAIT_PARAM5_DFT,WAIT_PARAM6_DFT,WAIT_PARAM7_DFT,MSG,"Set StartMode switch in auto mode. ")

if SkipGV:
	result("Operator skipped Start Mode AUTO instruction.",REPORT + "Start")

else:
	result("Start Mode set to AUTO",REPORT + "Start")

	pass


# ***** TESTING 003 PARA 2.A (1) (i) ***** Starter Air Valve (SAV) ON
...
# ***** TESTING 003 PARA 2.A (1) (j) ***** Engine Fuel Shutoff OFF
...
# ***** TESTING 003 PARA 2.A (1) (k) ***** Engine throttle to Idle position
...
if SkipGV:
	...
	result("Throttle PLA at 35 degree (GI) {} Start".format(report))
...
# ***** TESTING 003 PARA 2.A (2) *****  Set FUEL SHUTOFF to ON

instruction("Set FUEL CONDITION LEVER to RUN")

wait("FuelCondLvr=1",10,0.1,WAIT_PARAM3_DFT,WAIT_PARAM4_DFT,WAIT_PARAM5_DFT,WAIT_PARAM6_DFT,WAIT_PARAM7_DFT,MSG,"Fuel switch not on")
...
# *********    Check for positive and increasing oil pressure

wait("POIL>5",30,2.5,WAIT_PARAM3_DFT,WAIT_PARAM4_DFT,WAIT_PARAM5_DFT,WAIT_PARAM6_DFT,WAIT_PARAM7_DFT,MSG,"POIL did not reach 2.5 psig.")

if SkipGV:
	result("ABORT START due to no Oil pressure indication {}".format(red),REPORT + "Start")
...
if getCV("N1_OBS") <10:
	...
	result("ABORT START due to no N1 indication. {}".format(red),REPORT +"Start")

else:
	result("N1 = {} secs".format(str(round(getCV("N1_OBS"),4)) ),REPORT +"Start")

	pass


# ***** TESTING 003 PARA 2.A (3) *****  When core speed N2 is at Minimum Idle,
# *************** ENGINE AT IDLE ***********************
...
# ***** TESTING 003 PARA 2.A (4) ***** Stabilize at Minimum Idle for five minutes minimum.

delay(300)



# ***** TESTING 003 PARA 2.A (5) (d) ***** Interrogate the FADEC Interface Unit (FIU) for FADEC System faults.
